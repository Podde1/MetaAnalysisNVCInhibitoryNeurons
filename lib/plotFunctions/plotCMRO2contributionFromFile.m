function [] = plotCMRO2contributionFromFile()

load('Parameters/CellSpecificContributions/CMRO2Table.mat', 'CMRO2Table')

%% Load and sort data
Experiment   = cell( size(CMRO2Table{1,2},1)*size(CMRO2Table(1,2:end),2), 1, size(CMRO2Table,1) );
NeuronType   = cell( size(CMRO2Table{1,2},1)*size(CMRO2Table(1,2:end),2), 1, size(CMRO2Table,1) );
Contribution = zeros(size(CMRO2Table{1,2},1)*size(CMRO2Table(1,2:end),2), 1, size(CMRO2Table,1) );
...
CMRO2Table = CMRO2Table(rearrangeOrder,:);
ShortExperimentNames = ShortExperimentNames(:,rearrangeOrder);

for i = 1:size(CMRO2Table,1) % rows 
    count = 1;
    for j = 1:size(CMRO2Table(1,2:end),2) % columns
        for k = 1:size(CMRO2Table{1,2},1) % elements in array
            Experiment{count,1,i}   = CMRO2Table{i};
            NeuronType{count,1,i}   = NeuronTypes{j};
            Contribution(count,1,i) = CMRO2Table{i,j+1}{k};
            count = count + 1;
        end
    end
end
...
exportgraphics(gcf,'Figures/BoxplotCMRO2.png', 'Resolution', 600)
exportgraphics(gcf,'Figures/BoxplotCMRO2.pdf', 'ContentType', 'vector')
...
spiderMatrix = zeros(size(CMRO2Table,1), size(CMRO2Table(1,2:end),2));

for i=1:size(spiderMatrix,1)
    for j=1:size(spiderMatrix,2)
        spiderMatrix(i,j) = CMRO2Table{i,j+1}{1};
    end
end
...
exportgraphics(gcf,'Figures/SpiderCMRO2.png', 'Resolution', 600)
exportgraphics(gcf,'Figures/SpiderCMRO2.pdf', 'ContentType', 'vector')
