function [] = plotBOLDcontributionFromFile()

load('Parameters/CellSpecificContributions/BOLDTable.mat', 'BOLDTable')

%% Load and sort data
Experiment   = cell( size(BOLDTable{1,2},1)*size(BOLDTable(1,2:end),2), 1, size(BOLDTable,1) );
NeuronType   = cell( size(BOLDTable{1,2},1)*size(BOLDTable(1,2:end),2), 1, size(BOLDTable,1) );
Contribution = zeros(size(BOLDTable{1,2},1)*size(BOLDTable(1,2:end),2), 1, size(BOLDTable,1) );
...
BOLDTable = BOLDTable(rearrangeOrder,:);
ShortExperimentNames = ShortExperimentNames(:,rearrangeOrder);

for i = 1:size(BOLDTable,1) % rows 
    count = 1;
    for j = 1:size(BOLDTable(1,2:end),2) % columns
        for k = 1:size(BOLDTable{1,2},1) % elements in array
            Experiment{count,1,i}   = BOLDTable{i};
            NeuronType{count,1,i}   = NeuronTypes{j};
            Contribution(count,1,i) = BOLDTable{i,j+1}{k};
            count = count + 1;
        end
    end
end
...
for i = 1:numElements
    ...
    % Update objects
    set(ax,{'YLim','Color','XColor','YColor','LineWidth','FontSize', 'Box'},{[0 100], ExperimentColors(i,1:4), [0 0 0], [0 0 0], 2, 24, 'off'})
...
exportgraphics(gcf,'Figures/BoxplotBOLD.png', 'Resolution', 600)
exportgraphics(gcf,'Figures/BoxplotBOLD.pdf', 'ContentType', 'vector')
...
spiderMatrix = zeros(size(BOLDTable,1), size(BOLDTable(1,2:end),2));

for i=1:size(spiderMatrix,1)
    for j=1:size(spiderMatrix,2)
        spiderMatrix(i,j) = BOLDTable{i,j+1}{1};
    end
end
...
exportgraphics(gcf,'Figures/SpiderBOLD.png', 'Resolution', 600)
exportgraphics(gcf,'Figures/SpiderBOLD.pdf', 'ContentType', 'vector')
%