function [model] = Sten2023_v31_syms()
...
model.sym.xdot(5) =  + k_u4*u4*am_max(N_SOMMax-N_SOM,0) + kPF5*Glut - kIN5*GABA - sinkN_SOM*N_SOM;      % N_SOM

model.sym.xdot(6) = + kGABA_NO*Ca_NO + kGABA_NPY*Ca_NPY + kGABA_Astro*Ca_Astro + kGABA_SOM*Ca_SOM - sink_GABA*GABA; % GABA
model.sym.xdot(7) = + kGlut_Astro*Ca_Astro + kGlut_Pyr*Ca_Pyr - sink_Glut*Glut;                                     % Glutamate
...
model.sym.xdot(16) = kNOS2*Ca_SOM - kNO2     * SOM_NO /(Km_NO2  + SOM_NO);  % SOM_NO
model.sym.xdot(17) = kNPY2*Ca_SOM - Vmax_NPY2* SOM_NPY/(Km_NPY2 + SOM_NPY); % SOM_NPY

%% GABAergic NO signaling
model.sym.xdot(18) = kNOS*Ca_NO - kNO*(NO/(Km_NO + NO));     % NO
model.sym.xdot(19) = kNO*(NO/(Km_NO + NO)) + kNO2*(SOM_NO/(Km_NO2 + SOM_NO)) - sinkNO*NOvsm;   % NOvsm
...
stim_circ = ky1*(NOvsm-NOvsm0) + ky2*(PGE2vsm-PGE2vsm0) - ky3*(NPYvsm-NPYvsm0);
...
model.sym.xdot(22) = (1/vis1)*(((K1 -(V1/V1ss))/(K1-1)) + stim_circ - 2*(V1/(C1ss*((R1+R2)*f1+(R2+R3)*f2+R3*f3)))); % V1 - Arterial CBV
model.sym.xdot(23) = (1/vis2)*(((K2 -(V2/V2ss))/(K2-1))             - 2*(V2/(C2ss*((R2+R3)*f2+R3*f3))));            % V2 - Capillary CBV
model.sym.xdot(24) = (1/vis3)*(((K3 -(V3/V3ss))/(K3-1))             - 2*(V3/(C3ss*(R3*f3))));                       % V3 - Venous CBV

model.sym.xdot(25) = f0-(1/vis1)*(((K1 -(V1/V1ss))/(K1-1)) + stim_circ - 2*(V1/(C1ss*((R1+R2)*f1+(R2+R3)*f2+R3*f3))))-f1;% f1 - Arterial CBF
model.sym.xdot(26) = f1-(1/vis2)*(((K2 -(V2/V2ss))/(K2-1))             - 2*(V2/(C2ss*((R2+R3)*f2+R3*f3))))-f2;           % f2 - Capillary CBF
model.sym.xdot(27) = f2-(1/vis3)*(((K3 -(V3/V3ss))/(K3-1))             - 2*(V3/(C3ss*(R3*f3))))-f3;                      % f3 - Venous CBF
...
CMRO2 = CMRO2_0*(1 + kCMRO2_NO*(Ca_NO-model.sym.x0(8)) + kCMRO2_NPY*(Ca_NPY-model.sym.x0(9)) + kCMRO2_Pyr*(Ca_Pyr-model.sym.x0(10)) + kCMRO2_Astro*(Ca_Astro-model.sym.x0(11)) + kCMRO2_SOM*(Ca_SOM-model.sym.x0(12)));
...
%% M matrix
    ...
    matris(25,25) = 0;  matris(26,26) = 0; matris(27,27) = 0;   
...
%%
% OBSERVALES
syms CBF_y BOLD_OIS CBV_y HbT_y HbO_y HbR_y BOLD_fMRI CMRO2_y VasoactivEffect f0_y f1_y f2_y f3_y HbTrelative_y HbOrelative_y HbRrelative_y
model.sym.y = [CBF_y BOLD_OIS CBV_y HbT_y HbO_y HbR_y BOLD_fMRI CMRO2_y VasoactivEffect f0_y f1_y f2_y f3_y HbTrelative_y HbOrelative_y HbRrelative_y]; 

model.sym.y(1) = CBFprecent;           % fractional CBF
model.sym.y(2) = BOLD_OISpercent;           % fractional CBF
model.sym.y(3) = CBVpercent;   % precental CBV
model.sym.y(4) = HbTpercent; % precental Hemoglobin Total 
model.sym.y(5) = HbOpercent; % precental OxyHemoglobin  
model.sym.y(6) = HbRpercent; % precental DeOxyHemoglobin 

model.sym.y(7) = BOLDpercent;   % Precental BOLD fMRI - signal
model.sym.y(8) = CMRO2;
model.sym.y(9) = stim_circ;
model.sym.y(10) = f0;
model.sym.y(11) = f1;
model.sym.y(12) = f2;
model.sym.y(13) = f3;

model.sym.y(14) = HbTrelative;
model.sym.y(15) = HbOrelative;
model.sym.y(16) = HbRrelative;
