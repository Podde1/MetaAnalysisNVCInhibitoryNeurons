function [] = plotHbRcontributionFromFile()

load('Parameters/CellSpecificContributions/HbRTable.mat', 'HbRTable')

%% Load and sort data
Experiment   = cell( size(HbRTable{1,2},1)*size(HbRTable(1,2:end),2), 1, size(HbRTable,1) );
NeuronType   = cell( size(HbRTable{1,2},1)*size(HbRTable(1,2:end),2), 1, size(HbRTable,1) );
Contribution = zeros(size(HbRTable{1,2},1)*size(HbRTable(1,2:end),2), 1, size(HbRTable,1) );
...
for i = 1:size(HbRTable,1) % rows 
    count = 1;
    for j = 1:size(HbRTable(1,2:end),2) % columns
        for k = 1:size(HbRTable{1,2},1) % elements in array
            Experiment{count,1,i}   = HbRTable{i};
            NeuronType{count,1,i}   = NeuronTypes{j};
            Contribution(count,1,i) = HbRTable{i,j+1}{k};
            count = count + 1;
        end
    end
end
...
exportgraphics(gcf,'Figures/BoxplotHbR.png', 'Resolution', 600)
exportgraphics(gcf,'Figures/BoxplotHbR.pdf', 'ContentType', 'vector')
...
spiderMatrix = zeros(size(HbRTable,1), size(HbRTable(1,2:end),2));

for i=1:size(spiderMatrix,1)
    for j=1:size(spiderMatrix,2)
        spiderMatrix(i,j) = HbRTable{i,j+1}{1};
    end
end
...
exportgraphics(gcf,'Figures/SpiderHbR.png', 'Resolution', 600)
exportgraphics(gcf,'Figures/SpiderHbR.pdf', 'ContentType', 'vector')
